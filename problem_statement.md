...
In the first example, \(n=3\) is odd, so \(2A+4B\) can never equal \(3\); hence no solution exists and the answer is \(-1\).

In the first example, \(n=4\) so \(A+2B=2\). Since \(A\equiv 0\pmod 2\) and \(B\equiv 0\pmod 2\), the only way to satisfy \(A+2B=2\) is \(A=2, B=0\), and then \(A+B=2\) is divisible by \(p=2\); the answer is \(2\).

In the first example, \(n=24\) so \(A+2B=12\). With \(A\equiv 0\pmod 6\), possible values are \(A=0,6,12\), giving \((B=6),(B=3),(B=0)\) respectively; however, in all cases \(A+B\in\{6,9,12\}\), none of which is divisible by \(p=5\), so the answer is \(-1\).

In the first example, \(n=18\) so \(A+2B=9\). With \(A\equiv 0\pmod 3\) and \(B\equiv 0\pmod 3\), \(B\) can only be \(0\) or \(3\); \(B=3\) would give \(A=3\), so \(A+B=6\), which is divisible by \(p=1\), and it is also the minimum possible total, so the answer is \(6\).